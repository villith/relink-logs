...
- **Expecting an updater `.msi.zip` from a local build.** `bundle.targets` is `["msi"]` only. In CI, `.github/workflows/release.yaml` passes `-b msi,updater` and the bundler produces the updater zip itself — from the already-Authenticode-signed MSI — then minisigns it, so no step builds that artifact by hand. (Historical note: when `updater` was still a bundle target, `tauri build` exited non-zero on success because `TAURI_PRIVATE_KEY` was missing locally; that no longer happens.)
