...
- **Versions** are tracked in three places that must agree on release: `package.json`, `src-tauri/Cargo.toml`, and `src-tauri/tauri.conf.json`. The updater endpoint is the auto-generated `latest.json` published by the release workflow (`releases/latest/download/latest.json`); there is no `update.json` in the repo any more.
- **Releases require a `CHANGELOG.md` section.** `.github/workflows/release.yaml` runs `scripts/extract-changelog.mjs <version>` *before* tagging and fails the job when the version has no `## <version>` section — a tag without a release would block that version forever. The section body becomes the GitHub release body, which `tauri-action` copies into `latest.json` as the updater notes rendered by `src/components/UpdateNotes.tsx`. Changelog entries are written by humans, not generated.
