...
/// Gauge generated by ONE hit, attributed to the action that caused it.
///
...
/// it. This one is emitted from the damage path, where the causing action is in
/// hand — the game grants per-hit gauge in a per-character virtual that takes
/// the `DamageInstance`, so `action_id` is simply a field of it.
///
/// LOCAL PLAYER ONLY. A remote member's gauge is synced rather than computed
/// here, so no hit we can see produced it; the poll path remains the only
/// source for them.
#[derive(Serialize, Deserialize, Debug, Clone)]
