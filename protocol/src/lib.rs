...
/// Gauge generated by one hit, attributed to the hit that caused it.
///
...
/// no timing heuristic involved. Emitted only when the hit's SOURCE resolves
/// to the gaining player (players also gain gauge from TAKING hits; that
/// equality check keeps an enemy's move from captioning the victim's bar).
/// Rises with no parked hit (regen ticks, scripted awards) stay unattributed.
///
