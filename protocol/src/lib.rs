...
/// Gauge generated by a hit, attributed to the owning player's most recent hit.
///
...
/// it. This one is emitted from the gauge-UPDATE path: the game processes
/// damage on a worker-thread pool while gauge updates arrive on another thread
/// (live-verified, c0d06e1), so the hook's damage detour records each
/// player-attributable hit in a per-slot last-hit map and the gauge hook
/// attributes a player component's rise to that owner's most recent hit when
/// it falls within a short freshness window — the stun-message precedent,
/// hook-side. Stale, absent, or non-player rises stay unattributed.
///
/// LOCAL PLAYER ONLY in practice: only locally-processed hits populate the
/// map — a remote member's gauge is synced rather than computed here, so no
/// hit we can see produced it; the poll path remains the only source for them.
#[derive(Serialize, Deserialize, Debug, Clone)]
