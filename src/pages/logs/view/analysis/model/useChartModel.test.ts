...
describe("overlayOf", () => {
  it("is null when no builder produced a series", () => {
    ...
    expect(overlayOf(null, null, null, null)).toBeNull();
  });

  it("prefers the pinned effect's stacks over every other overlay", () => {
    expect(overlayOf(series("status"), series("effect"), series("group"), series("ability"))?.[0].key).toBe("status");
  });

  it("falls through the chain in order", () => {
    expect(overlayOf(null, series("effect"), series("group"), series("ability"))?.[0].key).toBe("effect");
    expect(overlayOf(null, null, series("group"), series("ability"))?.[0].key).toBe("group");
    expect(overlayOf(null, null, null, series("ability"))?.[0].key).toBe("ability");
  });
  ...
  it("returns the winning array BY REFERENCE", () => {
    ...
    expect(overlayOf(status, null, null, null)).toBe(status);
    const ability = series("ability");
    expect(overlayOf(null, null, null, ability)).toBe(ability);
  });
  ...
  it("treats an EMPTY series array as a real overlay, not as absent", () => {
    ...
    expect(overlayOf(empty, series("effect"), null, null)).toBe(empty);
  });
});
