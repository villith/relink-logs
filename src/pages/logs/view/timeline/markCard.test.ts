...
describe("markCardSections", () => {
  it("builds one section listing the mark's parts", () => {
    const sections = markCardSections(
      { startMs: 0, endMs: 100, count: 3, amount: 30, by: [{ key: "A", count: 2, amount: 20 }] },
  ...
  it("colours the section with the lane's own colour", () => {
    const sections = markCardSections(
      { startMs: 0, endMs: 0, count: 1, amount: 5, by: [{ key: "A", count: 1, amount: 5 }] },
      { color: "#F06595", entry: NAMES }
    );
  ...
  it("builds no section for a mark with no parts", () => {
    expect(
      markCardSections({ startMs: 0, endMs: 4000, count: 1, amount: null, by: [] }, { color: "#000", entry: NAMES })
    ).toEqual([]);
  });
